...
The sole common point between the authentication methods is the need to provide a `client_token` that can be used to authenticate with HashiCorp Vault.
...
The following settings have been moved from the HashiCorp Vault Extension to th HashiCorp Vault Token Auth Extension.
